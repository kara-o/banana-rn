...
export const requestResetToken = async (store, {
	...
	const endpoint = `http://localhost:3000/password_resets/${userIdentity}/`;
...
export const submitResetToken = async (store, {
	...
	const { userIdentity } = store.state;
	const endpoint = `http://localhost:3000/password_resets/${userIdentity}/${input}/`;
	try {
...
export const submitNewPassword = async (store, {
	...
	const endpoint = `http://localhost:3000/password_resets/${userIdentity}/${token}`;
	try {
